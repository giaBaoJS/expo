...
Per-screen metrics appear in the **Navigation** tab of the [EAS Observe dashboard](/eas/observe/dashboard/). From the terminal, query them by screen with `eas observe:routes`:

<Terminal
  cmd={[
    '# All navigation metrics, grouped by route',
    '$ eas observe:routes --days 7',
    '',
    '# Time to interactive for two routes',
    '$ eas observe:routes --metric nav_tti --route-name /home --route-name /checkout',
  ]}
/>
